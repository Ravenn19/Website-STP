...
<head>
  ...
  <title>List Data Prototipe</title>
...
<body class="flex min-h-screen bg-gradient-to-br from-white to-blue-400">
      ...
      <nav class="space-y-4">
        ...
        <div>
          <button onclick="toggleSubmenu('submenu-list', 'icon-list')" class="flex justify-between items-center w-full p-2 rounded hover:bg-blue-400">
            <span>List Data</span>
            <svg id="icon-list" class="w-3 h-3 transform transition-transform rotate-180" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" d="M5 15l7-7 7 7" />
            </svg>
          </button>
          <div id="submenu-list" class="submenu pl-4 space-y-2 show">
            ...
            <a href="{{ url('/admin2/list-prototipe') }}" class="block p-2 rounded bg-blue-500">Prototipe</a>
          </div>
        </div>
      </nav>
    </div>
  ...
  <div class="flex-1 p-8">
    ...
    <h1 class="text-3xl font-bold mb-6">List Data Prototipe</h1>
    ...
    <div class="overflow-auto bg-white rounded shadow-md">
      <table class="min-w-full text-sm text-left text-gray-800">
        ...
        <tbody>
          @forelse ($prototipeList as $no => $item)
            <tr class="text-center border-b hover:bg-gray-50">
              <td class="py-2 px-4">{{ $no + 1 }}</td>
              <td class="py-2 px-4">{{ $item->jenis_prototipe }}</td>
              <td class="py-2 px-4">{{ $item->judul_prototipe }}</td>
              <td class="py-2 px-4">{{ $item->nama_pemohon }}</td>
              <td class="py-2 px-4">{{ $item->tanggal_pengajuan }}</td>
              <td class="py-2 px-4">
                @if ($item->dokumen)
                  <a href="{{ asset('writable/uploads/' . $item->dokumen) }}" target="_blank" class="text-blue-600 underline">Lihat Dokumen</a>
                @else
                  -
                @endif
              </td>
              <td class="py-2 px-4">
                <a href="{{ url('/admin2//hapus-prototipe/' . $item->id) }}" class="text-red-600 hover:text-red-800" onclick="return confirm('Yakin ingin menghapus data ini?')">
                  Hapus
                </a>
              </td>
            </tr>
          @empty
            <tr><td colspan="7" class="py-4 text-center text-gray-500">Data kosong</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
