...
<body class="flex min-h-screen bg-gradient-to-br from-white to-blue-400">
      ...
      <nav class="space-y-4">
        ...
        <div>
          ...
          <div id="submenu-list" class="submenu pl-4 space-y-2 show">
            <a href="{{ url('/admin2/list-hki') }}" class="block p-2 rounded bg-blue-500">HKI</a>
  ...
  <div class="flex-1 p-8">
    ...
    <h1 class="text-3xl font-bold mb-6">List Data HKI</h1>
      ...
      <table class="min-w-full text-sm text-center text-gray-800">
        <thead class="bg-gray-200 text-gray-700">
          <tr>
            ...
            <th class="py-2 px-4 border-b">No Daftar</th>
            <th class="py-2 px-4 border-b">NDIN</th>
            <th class="py-2 px-4 border-b">Dokumen</th>
            <th class="py-2 px-4 border-b">Aksi</th>
          </tr>
        </thead>
        <tbody>
          @forelse ($hkiList as $index => $item)
            <tr class="text-center border-b hover:bg-gray-50">
              <td class="border px-4 py-2">{{ $index + 1 }}</td>
              <td class="border px-4 py-2">{{ $item->jenis_hki }}</td>
              <td class="border px-4 py-2">{{ $item->judul_hki }}</td>
              ...
              <td class="border px-4 py-2">{{ $item->ndin ?? '-' }}</td>
              ...
              <td class="py-2 px-4">
                <form action="{{ route('list.hki.delete', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                  @csrf
                  @method('DELETE')
          ...
          @empty
            <tr>
              <td colspan="9" class="py-4 text-center text-gray-500">Data kosong</td>
            </tr>
          @endforelse
        </tbody>
      </table>
