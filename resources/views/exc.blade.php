...
<head>
    ...
    <title>Penyewaan Exclusive Room</title>
...
<body style="">
    <div class="container">
        <div class="header">
            <h1>PENYEWAAN EXCLUSIVE ROOM</h1>
        ...
        <div class="form-container">
            ...
            <form id="excReservationForm" method="POST" action="{{ route('excl.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <select id="exc" name="exc" class="form-control" required>
                        <option value="" disabled selected>Select exclusive room</option>
                    </select>
                </div>
                ...
                <div class="checkbox-group">
                    ...
                    <label for="agreement">Saya menyetujui bahwa data yang saya isi benar dan bersedia mengikuti penggunaan exclusive room.</label>
                </div>
    ...
    <script>
        const labOptions = {
            exclusive: [
                "Ruang Pembangkit (Studio)",
                "Auditorium",
                "Ruang Prof. Dr. Ir. Sedyatmo",
                "Ruang Kantin Dosen"
            ]
        };

        const allExc = labOptions.exclusive;

        function populateExcOptions(excList) {
            const excSelect = document.getElementById('exc');
            excSelect.innerHTML = '<option value="" disabled selected>Pilih ruang exclusive</option>';
            excList.forEach(exc => {
                const option = document.createElement('option');
                option.value = exc.toLowerCase().replace(/[^a-z0-9]+/g, '_');
                option.textContent = exc;
                excSelect.appendChild(option);
            });
        }

        // Initialize on page load
        window.addEventListener('DOMContentLoaded', () => {
            populateExcOptions(allExc);
        });
    </script>
